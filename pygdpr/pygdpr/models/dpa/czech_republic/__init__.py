...
class CzechRepublic(DPA):
    ...
    def get_docs_CourtRulings(self, existing_docs=[], overwrite=False, to_print=True):
        ...
        results_soup = BeautifulSoup(page_source.text, 'html.parser')
        assert results_soup
        obalcelek_tag = results_soup.find('div', id='obalcelek')
        assert obalcelek_tag
        ui_tag = obalcelek_tag.find('ul', class_='ui')
        assert ui_tag
        ...
        for li in ui_tag.find_all('li'):
            ...
            document_response = None
            try:
                document_response = requests.request('GET', document_url, timeout=4)
                document_response.raise_for_status()
            except requests.exceptions.HTTPError as error:
                pass
            if document_response is None:
            ...
            link_obalcelek_tag = document_soup.find('div', id='obalcelek')
            assert link_obalcelek_tag
            link_ui_tag = link_obalcelek_tag.find('ul', class_='ui')
            assert link_ui_tag
            ...
            for link_li in link_ui_tag.find_all('li'):
                ...
                link_response = None
                try:
                    link_response = requests.request('GET', link_url, timeout=4)
                    link_response.raise_for_status()
                except requests.exceptions.HTTPError as error:
                    pass
                if link_response is None:
    ...
    def get_docs_DecisionMakingActivites(self, existing_docs=[], overwrite=False, to_print=True):
        print('------------ GETTING DECISION-MAKING ACTIVITIES ------------')
        iteration = 1
        added_docs = []
    ...
    def get_docs_DecisionOfPresident(self, existing_docs=[], overwrite=False, to_print=True):
        print('------------ GETTING DECISION OF PRESIDENT ------------')
        added_docs = []
