...
from pygdpr.services.summary_meta_service import SummaryMetaService
from pygdpr.services.timeline_meta_service import TimelineMetaService
from pygdpr.services.gdpr_article_refs_meta_service import GDPRArticleRefsMetaService
from pygdpr.services.gdpr_refs_count_meta_service import GDPRRefsCountMetaService
from pygdpr.services.keywords_meta_service import KeywordsMetaService
from pygdpr.services.char_length_meta_service import CharLengthMetaService
from pygdpr.services.word_length_meta_service import WordLengthMetaService
from pygdpr.services.sent_length_meta_service import SentLengthMetaService
from pygdpr.services.est_read_time_meta_service import EstReadTimeMetaService
...
class DPA(object):
        ...
        for root,dirs,files in os.walk(self.path + self.country.lower(), topdown=True):
            ...
            for name in files:
                ...
                with open(root + '/' + name, 'r', encoding='utf-8') as f:
                    ...
                    if NotReached100SecsTranslateQuotaSpec(quota_service).is_satisfied_by(next_quota) is False:
                        if to_print:
    ...
    def extract_metadata(self, pipeline=[
        ('summary', SummaryMetaService()),
        ('timeline', TimelineMetaService()),
        ('gdprArticleRefs', GDPRArticleRefsMetaService()),
        ('gdprRefsCount', GDPRRefsCountMetaService()),
        ('keywords', KeywordsMetaService()),
        ('charLength', CharLengthMetaService()),
        ('wordLength', WordLengthMetaService()),
        ('sentLength', SentLengthMetaService()),
        ('estReadTime', EstReadTimeMetaService())
    ], docs=[], to_print=True):
